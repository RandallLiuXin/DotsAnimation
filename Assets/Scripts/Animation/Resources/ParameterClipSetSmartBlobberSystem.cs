...
namespace Tinder.Animation.Authoring
{
    ...
    /// Animation Smart Blobbers only ever read this config, meaning it is safe to share an UnsafeList between multiple configs.
    /// </summary>
    ...
    /// All values must be allocated with the conversion world's UpdateAllocator.
    /// Animation Smart Blobbers only ever read this bake data, meaning it is safe to share an UnsafeList between multiple bake data instances.
    /// </summary>
    ...
    public class ParameterClipSetSmartBlobberSystem : SmartBlobberConversionSystem<ParameterClipSetBlob, ParameterClipSetBakeData, ParameterClipSetConverter>
    {
        ...
        private static bool FilterBursted(in ParameterClipSetBakeData input, in FixedString128Bytes gameObjectName)
        {
            ...
            foreach (var clip in input.clips)
            {
                ...
                {
                    Debug.LogError($"Animation failed to convert parameter clip \"{clip.name}\" on GameObject {gameObjectName} because the clipData was unitialized.");
                    return false;
                }
                if (parameterCount < 0)
                ...
                {
                    Debug.LogError(
                        $"Animation failed to convert parameter clip \"{clip.name}\" on GameObject {gameObjectName} because the length of clipData ({clip.clipData.Length}) did not match previous clip parameter count of {parameterCount}.");
                    return false;
                }
                ...
                foreach (var samples in clip.clipData)
                {
                    ...
                    {
                        Debug.LogError($"Animation failed to convert parameter clip \"{clip.name}\" on GameObject {gameObjectName} because some of its samples were unitialized.");
                        return false;
                    }
                    ...
                    {
                        Debug.LogError(
                            $"Animation failed to convert parameter clip \"{clip.name}\" on GameObject {gameObjectName} because the number of samples is not the same for all parameters.");
                        return false;
                    }
                }
                ...
                if (clip.maxErrorsByParameter.IsCreated)
                {
                    ...
                    {
                        Debug.LogError(
                            $"Animation failed to convert parameter clip \"{clip.name}\" on GameObject {gameObjectName} because the length of maxErrorsByParameter ({clip.maxErrorsByParameter.Length}) did not match the clip parameter count of {parameterCount}.");
                        return false;
                    }
                }
            }
            ...
            if (input.parameterNames.IsCreated)
            {
                ...
                {
                    Debug.LogError(
                        $"Animation failed to convert parameter clip set on GameObject {gameObjectName} because the length of parameterNames ({input.parameterNames.Length}) did not match the parameter count of {parameterCount}.");
                    return false;
                }
            }
